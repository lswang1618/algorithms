function displayResponseTable(question, responses, choices, readers){
    ...
    // Hide div before rending new one
    $(".dashboard-title-2").hide()
  ...
  if(responses.length > 0){
    ...
    for (var i = 0; i < responses.length; i++) { // iterate through all responses
        ...
        line += responses[i].article + ','; // article where answered
    ...
    download_link.textContent='Download list of responses as CSV';
    ...
    for(var i = 0; i < responses.length; i++){
      ...
      for(var j = 0; j < 1; j++){
        ...
        var td2 = tr.insertCell();
        td2.appendChild(document.createTextNode(responses[i].article));
      }
    }
